...
function calculate() {
  ...
  rows.forEach(row => {
    const inzet = parseFloat(row.children[1].children[0].value) || 0;
    const winst = parseFloat(row.children[2].children[0].value) || 0;
    const totaal = inzet + winst;

    row.querySelector(".rowTotal").innerText = totaal.toFixed(2);
    weekTotal += totaal;
  });
